...
ac
    ...
    for _ in range(n):
        l = residual_block(l,dropout=0.15)
    print ('first residual stack built')
    # second stack of residual blocks, output is 32 x 16 x 16
    l = residual_block(l, increase_dim=True,dropout=0.25)
    for _ in range(1,n):
        l = residual_block(l,dropout=0.4)
    print('second residual stack built')

    # third stack of residual blocks, output is 64 x 8 x 8
    l = residual_block(l, increase_dim=True,dropout=0.4)
    for _ in range(1,n):
        l = residual_block(l,dropout=0.4)
...
net = NeuralNet(
    ...
    regression=True,
    batch_iterator_train=FlipBatchIterator(batch_size=30),
    on_epoch_finished=[
